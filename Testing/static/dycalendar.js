...
(function (global) {
    ...
    function createMonthTable(data, option) {
        ...
        while (c <= 6) {
            td = document.createElement("td");
            td.innerHTML = count;
            td.setAttribute("class", "date" + count);
            if (data.today.date === count && data.today.monthIndex === data.monthIndex && option.highlighttoday === true) {
                td.setAttribute("class", td.getAttribute("class") + " dycalendar-today-date");
            }
    ...
    function drawCalendarMonthTable(data, option) {
        ...
        div.setAttribute("id", "dycalendar-body");
